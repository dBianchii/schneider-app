...
export function SavePost({ postId }) {
    ...
    return (
        <div className="p-3 rounded-md " >
            {isSaved ? <IoBookmark onClick={removePost} className={`ml-4 h-7 w-7 text-blue-400 `} /> :
                <IoBookmarkOutline onClick={savePost} className={`ml-4 h-7 w-7 text-gray-400 hover:text-blue-400`} />
            }
        </div>
    )
}