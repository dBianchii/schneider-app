      ...
      paginationItems.push(
        <li key={i}>
          <a
            ...
            } ml-0 border border-gray-300 bg-white px-3 py-2 leading-tight text-gray-500 hover:bg-gray-100 hover:text-gray-700`}
          >
            {i}
  ...
  return (
    <div class="max-w-sm rounded-lg border border-gray-200 bg-white shadow">
      ...
      <div class="p-5">
        ...
        <p class="mb-3 text-base font-normal text-gray-600">
          {post.description}
        </p>
        ...
        <div className="mt-4 flex flex-row align-middle">
          {user && post.likes.includes(user.id) ? (
            <AiFillHeart className="h-8 w-8 text-red-500 hover:text-red-500/80" />
          ) : (
            <AiOutlineHeart className="h-8 w-8 hover:text-red-500/80" />
          )}
          {post.likes.length > 0 && <span>{post.likes.length}</span>}
          <AiOutlineComment
            className={`ml-4 h-8 w-8 ${
              user && post.comments ? "text-gray-500" : ""
            }`}
          />
        </div>
      </div>
